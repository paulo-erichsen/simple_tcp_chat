...
require 'socket'
require './helper.rb'
require './colorize.rb' # from the 'colorize' gem
...
class Server
  ...
  def run
    ...
    loop do # kernel#loop -> repeatedly executes the block
      ...
      Thread.start(@server.accept) do | client |
        ...
        nick_name = client.readline.chomp.to_sym
        # check that the username is unique
        if nick_name.to_s.empty? || nick_name.to_s == ADMIN_NAME
          ...
          @connections[:clients].each do |other_name, other_client|
            if nick_name == other_name || client == other_client
              ...
              client.send 'This username already exist', 0
              client.close
  ...
  def handle_admin_input
    ...
    loop do
      ...
      when 'kick'
        user_name = input.split[1..-1].join(' ')
        log("kicking the user: #{user_name}", :debug)
        kick(user_name.to_sym) unless user_name.empty? # kick(usr)
  ...
  def kick(client_name)
    ...
    @connections[:clients][client_name].send msg, 0
  ...
  def send_room(room, message, client_name = nil, message_is_room_info = false)
    ...
    log("checking if room and message were passed in", :debug)
    unless room && message
      puts 'usage: send <room> <message>'
      return
    end

    log("checking if the room exists", :debug)
    ...
    log("changing the message, based on who sent it", :debug)
    ...
    if client_name
      ...
      message = ADMIN_TAG + message unless message.start_with?(ADMIN_TAG)
      # message.prepend(ADMIN_TAG) unless message.start_with?(ADMIN_TAG)
    end
    log("updated message: #{message}", :debug)

    # send the message to each client in the room (except for the given name)
    log("now about to send the message to some client(s)", :debug)
    @connections[:rooms][room].each do |client_nam3|
      if client_nam3 != client_name
        log("sending message to the following client: #{client_nam3}", :debug)
    ...
    log("checking if the admin is in the room", :debug)
  ...
  def send_client(client_name, message,
    ...
    log("send_client - changing the message based on who sent it", :debug)
    if from_client_name
      # message.prepend("[private - #{from_client_name}]: ")
      message = "[private - #{from_client_name}]: " + message
    elsif from_admin
      # message.prepend(ADMIN_TAG)
      message = ADMIN_TAG + message
    end
    log("updated message: #{message}", :debug)

    # if the destination is valid, then send it
    log("checking the destination and attempting to send it", :debug)
    ...
    log("we have reached the end of send_client", :debug)
  end
  ...
  def listen_user_messages(client_name, client_fd)
    loop do
      ...
      message = client_fd.readline.chomp
      msg = message.split
      log("user's input: #{message}", :debug)
      if command?(msg[0])
        log("user's input is a command", :debug)
        case msg[0]
        when '%p'
        ...
        log("sending the input to the following room: #{room.first}", :debug)
